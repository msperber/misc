...
class BiCompactLSTMBuilder:
  ...
  def transduce(self, es):
    ...
    for layer_i in range(1, len(self.forward_layers)):
      new_forward_es = ExpressionSequence(self.forward_layers[layer_i].initial_state().transduce([forward_es, ReversedExpressionSequence(rev_backward_es)]).as_list(), mask=mask)
