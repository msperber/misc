...
class BaseTrainingTask(TrainingTask):
  def __init__(self,
               model: 'model_base.TrainableModel',
...
class ConditionedTrainingTask(BaseTrainingTask, Serializable):
  """
  Args:
    model: a trainable conditioned model
  ...
  @serializable_init
  def __init__(self,
               model: 'model_base.ConditionedModel' = Ref("model"),
    ...
    super().__init__(model=model, batcher=batcher, run_for_epochs=run_for_epochs, loss_calculator=loss_calculator,
                     ...
                     lr_decay_times=lr_decay_times, patience=patience, initial_patience=initial_patience,
...
class UnconditionedTrainingTask(BaseTrainingTask, Serializable):
  """
  Args:
    model: a trainable unconditioned model
    trg_file: The file for the target data.
    dev_every: dev checkpoints every n sentences (0 for only after epoch)
    batcher: Type of batcher
    loss_calculator:
    ...
    name: will be prepended to log outputs if given
  """
  yaml_tag = '!UnconditionedTrainingTask'

  @serializable_init
  def __init__(self,
               model: 'model_base.UnconditionedModel' = Ref("model"),
               trg_file: str = None,
               dev_every: int = 0,
               batcher: batchers.Batcher = bare(batchers.SrcBatcher, batch_size=32),
               loss_calculator: loss_calculators.LossCalculator = bare(loss_calculators.MLELoss),
