...
class SimpleTrainingRegimen(TrainingRegimen, Serializable):
  """
  A simple single-task training regimen.
  ...
  @serializable_init
  def __init__(self,
               task: train_tasks.TrainingTask = bare(train_tasks.ConditionedTrainingTask),
    ...
    self.train_loss_tracker = TrainLossTracker(task)
    self.loss_comb_method = loss_comb_method

  def run_training(self, save_fct):
    if self.task.run_for_epochs > 0:
      for src, trg in self.task.next_minibatch():
        if self.dev_zero:
        ...
        with utils.ReportOnException({"src": src, "trg": trg, "graph": dy.print_text_graphviz}):
          ...
          with self.train_loss_tracker.time_tracker:
            ...
            loss_builder = self.task.training_step(src, trg)
        ...
        if self.task.checkpoint_needed():
          self.checkpoint_and_save(self.task, save_fct)
        if self.task.should_stop_training(): break

  def checkpoint_and_save(self, cur_task, save_fct):
    should_save = self.task.checkpoint(control_learning_schedule=True) # TODO: control_learning_schedule=True correct?
    if should_save:
      save_fct()
