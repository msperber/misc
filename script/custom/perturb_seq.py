...
Given reference sentences, sample corrupted sentences by first drawing a targeted distance from a Poisson distribution,
then for each edit sampling either an insertion, deletion, or substitution.
By default, (#sub,#ins,#del) is sampled uniformly from a simplex such that trg_dist = #sub + #ins + #del.
Optionally, integer-valued weights can be specified for the 3 operations, in which case the we sample from a
...
Usage:
  perturb_seq.py [options] <tau> <in.txt> [<out.txt>]
